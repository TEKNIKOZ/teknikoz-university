  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <section class="bg-gradient-to-r from-brand to-brand/80 text-white py-16 sm:py-20">
      <div class="container max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <p class="text-base sm:text-lg text-center text-white/90 max-w-3xl mx-auto">
          Industry-aligned projects that showcase your skills and build your portfolio
        </p>
      </div>
    </section>
    ...
    <section class="py-12 sm:py-16">
      <div class="container max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div v-if="activeCategory !== 'all'" class="mb-12">
          ...
          <div v-if="activeCategory === 'PLM Windchill'" id="windchill" class="scroll-mt-20"></div>
          
          <!-- Siemens Teamcenter Section -->
          <div v-if="activeCategory === 'Siemens Teamcenter'" id="teamcenter" class="scroll-mt-20"></div>
          
          <!-- Cloud Solutions Section -->
          <div v-if="activeCategory === 'Cloud Solutions'" id="cloud" class="scroll-mt-20"></div>
          
          <!-- AI/ML Section -->
          <div v-if="activeCategory === 'AI/ML'" id="aiml" class="scroll-mt-20"></div>
          
          <h2 :id="activeCategory.toLowerCase().replace(/[\s/]/g, '')" class="text-xl sm:text-2xl font-bold text-gray-900 mb-6 flex items-center scroll-mt-20">
            <div
              :class="[
            ...
            >
              <div class="p-4 sm:p-6">
                ...
                <div v-if="project.tools" class="mb-3">
                  <h4 class="text-xs font-semibold text-gray-700 mb-1">Tools:</h4>
                  <p :class="['text-xs font-medium', getCategoryTextColor(activeCategory)]">
                    {{ project.tools }}
                  </p>
                </div>
            ...
            >
              <div class="p-4 sm:p-6">
                <div class="flex items-start mb-3">
                  ...
                  <div class="flex-1">
                    <h4 class="font-bold text-gray-900 mb-2 text-sm sm:text-base">
                      {{ project.title }}
                    </h4>
        ...
        <div v-if="activeCategory === 'all'">
          ...
          >
            <!-- Technology Header -->
            <div class="mb-8">
              <h2 :id="category.id.toLowerCase().replace(/[\s/]/g, '')" class="text-xl sm:text-2xl font-bold text-gray-900 mb-3 flex items-center scroll-mt-20">
                <div
                  :class="[
                    'w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center mr-3',
                    getCategoryBgColor(category.id),
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              >
                <div class="p-4 sm:p-6">
                  ...
                  <div v-if="project.tools" class="mb-3">
                    <h4 class="text-xs font-semibold text-gray-700 mb-1">Tools:</h4>
                    <p :class="['text-xs font-medium', getCategoryTextColor(category.id)]">
                      {{ project.tools }}
                    </p>
                  </div>
...
useHead({
  title: 'Real Projects - TEKNIKOZ University',
  meta: [
    {
      name: 'description',
      content: 'Explore our comprehensive project portfolio across PLM Windchill, Siemens Teamcenter, Cloud Solutions, and AI/ML technologies'
    }
  ]
})
  ...
  'PLM Windchill': {
    visible: [
      ...
      },
      {
        ...
        description: 'Create automated workflows for Change Request → Change Notice using routing rules, role assignments & validation.',
    ...
    more: [
      {
        ...
        description: 'Integrate Altium/Creo data with Windchill using XPLM and verify BOM sync.'
      },
      {
        title: 'Supplier BOM Collaboration',
        description: 'Create a secure ProjectLink workspace for supplier BOM upload, redlining, and comparison.'
      },
      {
        title: 'Variant BOM for Electric Vehicles',
        description: 'Implement Option Sets, Variant Specs, and configurable BOM for EV product lines.'
      },
      {
        title: 'PLM-ERP Interface (SAP Integration)',
        description: 'Use Windchill ESI to simulate part and BOM publishing to SAP mock system.'
      },
      {
        title: 'Product Classification Implementation',
        description: 'Create classification hierarchies with search attributes using Windchill Classification module.'
      },
      {
        title: 'CAPA Documentation Flow in Medical Device PLM',
        description: 'Implement complaint intake, investigation doc linking, change control, and audit trail.'
      },
      {
        title: 'MPMLink Manufacturing Process Plan',
        description: 'Map MBOM to Process Plan and Resource Management in MPMLink for a small assembly.'
      },
      {
        title: 'CAD Data Management Strategy',
        description: 'Manage revisions, iterations, and release of Creo and SolidWorks files using Windchill Workgroup Manager.'
      }
    ]
  },
  'Siemens Teamcenter': {
    visible: [
      ...
      },
      {
        ...
        description: 'Design and deploy multi-level approval workflows using Teamcenter Process Designer.',
        tools: 'Teamcenter Process Designer, Workflow Templates, Business Rules'
      }
    ],
    more: [
      {
        title: 'BMIDE Customization',
        description: 'Extend data model, define new item types, property mappings, and create LOVs.'
      },
      {
        title: 'Product Configurator Use Case',
        description: 'Implement a variant configuration use case using effectivity and feature filters.'
      },
      {
        title: 'Active Workspace UI Customization',
        description: 'Customize tiles, commands, and add a custom info panel in Active Workspace.'
      },
      {
        title: 'JT2Go Visualization Integration',
        description: 'Enable lightweight JT visualization in Teamcenter and configure viewer access.'
      },
      {
        title: 'Teamcenter-SAP Integration Simulation',
        description: 'Simulate part and BOM transfer using BOP Interface mapping.'
      },
      {
        title: 'Classification Structure for Automotive Parts',
        description: 'Build a hierarchical classification with properties, images, and usage references.'
      },
      {
        title: 'Engineering Document Vault Setup',
        description: 'Configure metadata-driven document types, templates, and secure vaulting.'
      },
      {
        title: 'Teamcenter Data Migration (Legacy to Unified)',
        description: 'Plan and execute mock migration using import tools (TcIC or Excel Import).'
      }
    ]
  },
  'Cloud Solutions': {
    visible: [
      ...
      },
      {
        ...
        description: 'Create a DevOps pipeline (GitHub Actions/Azure DevOps) for BMIDE/Windchill code deployment.',
    ...
    more: [
      {
        ...
        description: 'Script AWS infra (EC2, VPC, RDS) using Terraform modules.'
      },
      {
        title: 'Secure PLM Deployment with VPN & IAM',
        description: 'Setup S2S VPN, IAM roles, and bucket policies for a secure PLM architecture.'
      },
      {
        title: 'Azure Kubernetes for PLM Microservices',
        description: 'Deploy a containerized PLM microservice (e.g., Document Service) using AKS.'
      },
      {
        title: 'S3-based Backup & Restore Plan',
        description: 'Automate daily backups of Windchill file vaults and DB to encrypted S3 buckets.'
      },
      {
        title: 'Cloud Monitoring with Grafana + Prometheus',
        description: 'Monitor CPU, Memory, Uptime of PLM infrastructure with alerts on Teams.'
      },
      {
        title: 'Hybrid On-Prem & Cloud PLM Sync',
        description: 'Simulate a hybrid architecture with file vault sync between on-prem server and S3.'
      },
      {
        title: 'Disaster Recovery Drill for PLM',
        description: 'Simulate infra loss and restore from backup with test validation plan.'
      },
      {
        title: 'Cost Optimization & Auto-scaling of PLM',
        description: 'Enable auto-scale, schedule stop/start, and analyze billing with AWS Cost Explorer.'
      }
    ]
  },
  'AI/ML': {
    visible: [
      ...
      },
      {
        ...
        description: 'Use OpenAI API to create an assistant that summarizes and answers queries from BOM/spec sheets.',
    ...
    more: [
      {
        ...
        description: 'Train a BERT model to classify CRs as Minor, Major, or Critical.'
      },
      {
        title: 'Anomaly Detection in PLM Logs',
        description: 'Use Isolation Forest to flag abnormal user/system activity in PLM audit logs.'
      },
      {
        title: 'Vision AI: Defect Detection in Parts',
        description: 'Build a CNN model to detect defects from part images (automotive quality use case).'
      },
      {
        title: 'AI for Document Auto-Filling',
        description: 'Train a model to auto-fill metadata in spec sheets using OCR + NLP.'
      },
      {
        title: 'AI Model Deployment with FastAPI',
        description: 'Create a microservice to host ML model for BOM recommendation.'
      },
      {
        title: 'Vector DB Search over Technical PDFs',
        description: 'Use FAISS + LangChain to allow smart semantic search over PLM manuals.'
      },
      {
        title: 'Chatbot for Supplier Onboarding',
        description: 'Create a chatbot that guides suppliers through PLM onboarding.'
      },
      {
        title: 'AI-based Root Cause Analysis',
        description: 'Build a model that correlates failure logs with design attributes to suggest probable root causes.'
      }
    ]
  }
}
...
const getCategoryDescription = (categoryId) => {
  const descriptions = {
    'PLM Windchill': 'Configuration Management, BOM, Change Processes, and ECAD/MCAD integration projects',
    'Siemens Teamcenter': 'BOM Management, MRO, Workflow automation, and deployment strategy projects',
    'Cloud Solutions': 'AWS/Azure PLM hosting, DevOps pipelines, and infrastructure automation projects',
    'AI/ML': 'Applied intelligence for Industry 4.0, automation, and engineering workflow optimization',
  }
